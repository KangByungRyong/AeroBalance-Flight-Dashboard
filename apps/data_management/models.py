...
class AbsimTrackLog(models.Model):
    ...
    # Dashboard가 텔레메트리를 아직 못 받은 Model(연결 안 됨/미가동)에 대해
    # null을 내려줄 수 있음(`docs/ABFAP_Export_Interface.md` §1-3 스펙은 non-null
    # double이나 실제 응답은 다름) — 값을 지어내지 않고 NULL로 보존(§1 원칙과 동일)
    lat = models.FloatField("위도", null=True, blank=True)
    lon = models.FloatField("경도", null=True, blank=True)
    alt = models.FloatField("고도 (ft)", null=True, blank=True)
    hdg = models.FloatField("헤딩 (°)", null=True, blank=True)
    pitch = models.FloatField("피치 (°)", null=True, blank=True)
    roll = models.FloatField("롤 (°)", null=True, blank=True)
    spd = models.FloatField("지상속도 (kt)", null=True, blank=True)
    tas = models.FloatField("TAS (kt)", null=True, blank=True)
    cas = models.FloatField("CAS (kt)", null=True, blank=True)
